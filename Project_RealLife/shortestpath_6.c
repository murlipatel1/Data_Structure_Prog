...
void display(int firstMatrix[][100], int rowFirst, int columnSecond);
struct Edge
{
    int u;
    int v;
...
void writeToFile()
{
    ...
    {
        fprintf(fptr, "Name of head child %s data of node of head child %d", g->V, g->E);
        while (g->edge != NULL)
        {
            fprintf(fptr, "\nName  %s\nno of int data %d\n", g->edge->u, g->edge->v);
        }
...
// Funtion that implements MIN_PATH's single source shortest path algorithm
...
void MIN_PATH(int graph[VER][VER], int src)
{
    ...
    int sptSet[VER]; // sptSet[i] will 1 if vertex i is included in shortest path tree or shortest distance from src to i is finalized
    ...
    for (count = 0; count < VER - 1; count++)
    {
        ...
        int u = minDistance(dist, sptSet); // u is always equal to src in first iteration.
